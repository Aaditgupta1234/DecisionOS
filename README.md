...
flowchart TD
    subgraph Ingestion["1. Ingestion & Data Quality"]
        A[Raw Dataset Upload CSV/JSON] --> B[Schema Inference & Mapping]
        B --> C[Data Quality & Completeness Audit]
    end

    subgraph Diagnostics["2. Diagnostic Intelligence & RCA"]
        C --> D[Deterministic KPI Engine]
        D --> E[Diagnostic Rule Finding Engine]
        E --> F[Causal Root Cause Analysis DAG]
    end

    subgraph Execution["3. Strategic Execution & Governance"]
        F --> G[Programs & Initiatives Management]
        G --> H[Milestone Velocity & Critical Path Engine]
        H --> I[Stage-Gate Governance & Compliance]
        I --> J[Benefits Realization & ROI Tracking]
    end

    subgraph StrategicAnalytics["4. Strategic Analytics (Phase 12.7)"]
        J --> K[Strategic Value & Maturity Indexing]
        K --> L[Executive Attention Queue & Rankings]
        L --> M[Portfolio Value & Cohort Diagnostics]
    end

    subgraph Historical["5. Historical Intelligence (Phase 12.8)"]
        M --> N[Cryptographic Snapshot Persistence SHA-256]
        N --> O[Historical State Replay Engine]
        O --> P[Rolling Window Analytics 7d-365d]
    end

    subgraph DecisionSupport["6. Executive Decision Support (Phase 12.9)"]
        P --> Q[Portfolio Decision Readiness & Actionability]
        Q --> R[Investment Prioritization & Capacity]
